...
let main =
  ...
  Fibre.fork ~sw
    ...
    let x = Eio.Domain_manager.run (Eio.Stdenv.domain_mgr _env) (fun () -> (fib_par pool n)) in
          traceln "Parallel Fibonacci is done";
          let t2 = Eio.Time.now clock in
     traceln "Parallel Answer %d Time Difference for Parallel fibonacci %f" x (t2-.t1)
    );
  ...
  (fun () -> 
    ...
    let t1 = Eio.Time.now clock in
    (* traceln "Sequential Fibonacci = %d" (fib 45);    *)
    traceln "Sequential Fibonacci =" ;   
    let t2 = Eio.Time.now clock in
